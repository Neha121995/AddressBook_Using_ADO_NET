    ...
    class AddressBookRepo
    {
            ...
            try
            {
                ...
                using (this.connection)
                {
                    using (SqlCommand command = new SqlCommand(
                        @"SELECT * FROM address_books WHERE city = 'Satara' OR state = 'Maharashtra'; 
                            SELECT * FROM address_books WHERE city = 'Pune' OR state = 'Maharashtra';", connection))
                    {
                        connection.Open();
        ...
        public void AddressBookSizeByCityANDState()
        {
            try
            {
                ...
                {
                    using (SqlCommand command = new SqlCommand(
                        @"select count(first_name) from address_books WHERE city = 'Sangali' AND state = 'Maharashtra'; 
                        select count(first_name) from address_books WHERE city = 'Pune' AND state = 'Maharashtra';", connection))
                    {
                        connection.Open();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                Console.WriteLine("Total Contacts From City Sangali And State Maharashtra: {0}", +count);
                            }
                            if (reader.NextResult())
                            {
                                while (reader.Read())
                                {
                                    int count = reader.GetInt32(0);
                                    Console.WriteLine("Total Contacts From City Pune And State Maharashtra: {0}", +count);
                                }
                            }
                        }
                        connection.Close();
        ...
        public void SortPersonNameByCity()
        {
            try
            {
                ...
                {
                    using (SqlCommand command = new SqlCommand(
                        @"SELECT * FROM address_books WHERE city = 'Sangali' order by first_name; 
                        SELECT * FROM address_books WHERE city = 'Pune' order by first_name, last_name;", connection))
                    {
                        connection.Open();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
